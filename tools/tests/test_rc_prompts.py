...
class TestPromptManagerDefaults:
    def test_all_stages_present(self) -> None:
        """All LLM-driven stages in the 16-stage toolbox have prompts."""
        pm = PromptManager()
...
class TestWorkspaceAgents:
    def test_finds_nearest_agents_file(self, tmp_path: Path) -> None:
        outer = tmp_path / "AGENTS.md"
        outer.write_text("outer rules", encoding="utf-8")
        nested = tmp_path / "project" / "run"
        nested.mkdir(parents=True)
        inner = nested.parent / "AGENTS.md"
        inner.write_text("inner rules", encoding="utf-8")

        assert find_agents_file(nested) == inner

    def test_injects_workspace_rules_into_stage_and_sub_prompt(
        self, tmp_path: Path
    ) -> None:
        agents = tmp_path / "AGENTS.md"
        agents.write_text("AUTHORITATIVE_RULE_123", encoding="utf-8")
        pm = PromptManager(agents_search_start=tmp_path)

        stage_prompt = pm.for_stage("topic_init", topic="test")
        repair_prompt = pm.sub_prompt(
            "code_repair",
            fname="main.py",
            issues_text="failure",
            all_files_ctx="",
        ...
        assert "AUTHORITATIVE_RULE_123" in stage_prompt.user
        assert "AUTHORITATIVE_RULE_123" in repair_prompt.user
        assert pm.agents_path == agents
        assert len(pm.agents_sha256) == 64

    def test_extra_prompt_does_not_replace_stage_directive(
        self, tmp_path: Path
    ) -> None:
        agents = tmp_path / "AGENTS.md"
        agents.write_text("workspace rule", encoding="utf-8")
        pm = PromptManager(
            extra_prompts={"topic_init": "extra hint"},
            agents_search_start=tmp_path,
        )

        prompt = pm.for_stage("topic_init", topic="test")

        assert pm.directive("topic_init")
        assert "## AGENTS.md Directives" in prompt.user
        assert "extra hint" in prompt.user
        assert prompt.user.endswith("workspace rule")
