...
class SecurityConfig:
    hitl_required_stages: tuple[int, ...] = (5, 8, 10)
...
class ColliderAgentConfig:
    ...
    Workflow:
    1. Stage 11 generates a detailed physics prompt (collider_plan.md)
    2. Stage 13 invokes ``claude -p`` with ColliderAgent skills mounted
    ...
    magnus_address: str = ""
    magnus_token: str = ""
    # Incremental re-entry: when True, Stage 13 preserves the existing
...
class BiologyAgentConfig:
    ...
    Workflow:
    1. Stage 11 generates a biology execution prompt (biology_plan.md).
    2. Stage 13 invokes ``claude -p`` with Biology-Agent skills mounted.
...
class ExperimentRepairConfig:
    ...
    When enabled, after Stage 15 (result_analysis) the pipeline:
...
class MetaClawPRMConfig:
    ...
    gate_stages: tuple[int, ...] = (5, 8, 10, 16)
        ...
        return cls(
            ...
            security=SecurityConfig(
                hitl_required_stages=tuple(
                    int(s) for s in security.get("hitl_required_stages", (5, 8, 10))
                ),
...
def _parse_metaclaw_bridge_config(data: dict[str, Any]) -> MetaClawBridgeConfig:
    ...
    return MetaClawBridgeConfig(
        ...
        prm=MetaClawPRMConfig(
            ...
            gate_stages=tuple(
                int(s) for s in prm_data.get("gate_stages", (5, 8, 10, 16))
            ),
        ),
