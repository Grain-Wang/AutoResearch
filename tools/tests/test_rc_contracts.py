...
def test_contracts_dict_has_exactly_16_entries():
...
def test_gate_stages_have_expected_max_retries():
    ...
    assert CONTRACTS[Stage.BASELINE_REPRODUCE].max_retries == 0
...
def test_research_decision_contract_has_expected_outputs():
    contract = CONTRACTS[Stage.RESEARCH_DECISION]

    assert contract.output_files == ("decision.md",)
...
def test_stage_output_satisfies_next_input():
    ...
    #     wiring runtime real.
    # (b) TOPIC_INIT has no declared input because it is the pipeline entry.
