...
from researchclaw.llm.governance import GovernedLLMClient
...
# ---------------------------------------------------------------------------
# Stage 8 (extracted to stage_impls/_reproduce_defect.py)
# ---------------------------------------------------------------------------
from researchclaw.pipeline.stage_impls._reproduce_defect import (  # noqa: E402
    _execute_baseline_reproduce,
)

# ---------------------------------------------------------------------------
# Stages 7 & 9 (extracted to stage_impls/_synthesis.py; stage 8 = baseline_reproduce)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 10 (extracted to stage_impls/_experiment_design.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stage 11 (extracted to stage_impls/_code_generation.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stages 12-14 (extracted to stage_impls/_execution.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Stages 15-16 (extracted to stage_impls/_analysis.py)
# ---------------------------------------------------------------------------
...
_STAGE_EXECUTORS: dict[Stage, Callable[..., StageResult]] = {
    ...
    Stage.BASELINE_REPRODUCE: _execute_baseline_reproduce,
    ...
    *,
    ...
    try:
        ...
        prompts = PromptManager(
            ...
            agents_search_start=run_dir,
        )
        agents_context = {
            "source": str(prompts.agents_path) if prompts.agents_path else None,
            "sha256": prompts.agents_sha256 or None,
            "bundled_stage_directives": len(prompts.directives()),
        }
        (stage_dir / "agents_context.json").write_text(
            json.dumps(agents_context, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        if prompts.agents_bytes:
            # Agent-based experiment backends may only see the stage workspace.
            # Keep an exact snapshot there so nested tools inherit the same rules.
            (stage_dir / "AGENTS.md").write_bytes(prompts.agents_bytes)
        if llm is not None and prompts.agents_text:
            # Some legacy helpers call llm.chat() directly instead of rendering
            # through PromptManager. Enforce AGENTS.md at the shared boundary too.
            llm = GovernedLLMClient(llm, prompts.agents_text)
        try:
            result = executor(
    ...
    try:
        ...
        if (
            ...
            if mc_prm and getattr(mc_prm, "enabled", False):
                prm_stages = getattr(mc_prm, "gate_stages", (5, 8, 10, 16))
