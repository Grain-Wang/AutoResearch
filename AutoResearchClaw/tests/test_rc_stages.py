...
def test_stage_enum_has_exactly_16_members():
    assert len(Stage) == 16
...
def test_stage_sequence_contains_all_16_stages_in_order():
    assert len(STAGE_SEQUENCE) == 16
    assert STAGE_SEQUENCE[0] is Stage.TOPIC_INIT
    assert STAGE_SEQUENCE[-1] is Stage.RESEARCH_DECISION
...
def test_next_stage_boundary_values():
    ...
    assert NEXT_STAGE[Stage.RESEARCH_DECISION] is None
...
def test_gate_rollback_map_matches_expected_targets():
    assert GATE_ROLLBACK == {
        ...
        Stage.BASELINE_REPRODUCE: Stage.SYNTHESIS,
        Stage.EXPERIMENT_DESIGN: Stage.HYPOTHESIS_GEN,
        Stage.CODE_GENERATION: Stage.EXPERIMENT_DESIGN,
    }


def test_phase_map_has_6_phases_with_expected_membership():
    assert len(PHASE_MAP) == 6
    ...
    assert PHASE_MAP["C: Knowledge Synthesis & Defect Baseline"] == (
        Stage.SYNTHESIS,
        Stage.BASELINE_REPRODUCE,
    )
    assert PHASE_MAP["D: Hypothesis & Experiment Design"] == (
...
def test_reject_event_rolls_back_to_default_gate_mapping():
    outcome = advance(
        ...
        hitl_required_stages=(5, 8, 10),
    )

    assert outcome.status is StageStatus.PENDING
    assert outcome.stage is Stage.SYNTHESIS
    assert outcome.next_stage is Stage.SYNTHESIS
    assert outcome.rollback_stage is Stage.SYNTHESIS
    assert outcome.checkpoint_required is True
...
def test_reject_event_uses_explicit_rollback_stage_when_provided():
    outcome = advance(
        ...
        StageStatus.BLOCKED_APPROVAL,
        TransitionEvent.REJECT,
        rollback_stage=Stage.HYPOTHESIS_GEN,
    )

    assert outcome.status is StageStatus.PENDING
    assert outcome.stage is Stage.HYPOTHESIS_GEN
    assert outcome.next_stage is Stage.HYPOTHESIS_GEN
    assert outcome.rollback_stage is Stage.HYPOTHESIS_GEN
...
def test_gate_required_is_false_for_non_gate_stages(stage: Stage):
    assert gate_required(stage, (5, 8, 10)) is False
...
def test_default_rollback_stage_for_unknown_stage_uses_previous_stage():
    assert default_rollback_stage(Stage.RESULT_ANALYSIS) is Stage.ITERATIVE_REFINE
