"""Stages 15-16: Result analysis and research decision."""
...
def _execute_result_analysis(
    ...
    *,
    ...
    # --- R13-1: Merge Stage 14 (ITERATIVE_REFINE) results if available ---
    # Stage 14 stores richer per-condition metrics in refinement_log.json
    ...
    if _refine_log_text:
        try:
            ...
            if _best_iter is not None:
                ...
                # BUG-165 fix: Prefer Stage 14 refinement data when it is
                ...
                if not _refine_is_better and _refine_metrics:
                    ...
                    if _existing_pm is None:
                        ...
                        if _mdir == "maximize":
                            ...
                            _refine_is_better = _refine_pm < _existing_pm
                    logger.info(
                        "Stage 15: Refine metric comparison: existing=%s, refine=%s, "
            ...
            logger.warning("R13-1: Failed to parse refinement_log.json, using Stage 13 data")
    ...
    # First: from _collect_experiment_results (Stage 13 runs/)
    if exp_data.get("paired_comparisons"):
        _all_paired.extend(exp_data["paired_comparisons"])
    # Second: from refinement_log iterations (Stage 14)
    if _refine_log_text:
        try:
    ...
    # stage-11/12 to ensure multi-condition execution. The resulting
    ...
    if not _condition_summaries and _best_metrics:
        ...
        if _default_metrics:
            _condition_summaries["default_condition"] = {
                ...
                "_note": "stage-11/12 produced single-run output; "
...
"""
    ...
    # IMP-6 + FA: Generate charts early (Stage 15) so paper draft can reference them
    ...
    if config.experiment.figure_agent.enabled and llm is not None:
        try:
            ...
            if _fa_plan.figure_count > 0:
                ...
                logger.info(
                    "Stage 15: FigureAgent generated %d charts (%d passed review, %.1fs)",
                    _fa_plan.figure_count,
                    _fa_plan.passed_count,
                    _fa_plan.elapsed_sec,
                )
            else:
                logger.warning("Stage 15: FigureAgent produced no charts, falling back")
        except Exception as _fa_exc:
            logger.warning("Stage 15: FigureAgent failed (%s), falling back to visualize.py", _fa_exc)
    ...
    if not _figure_plan_saved:
        try:
            ...
            if _early_charts:
                ...
                logger.info(
                    "Stage 15: Generated %d early charts (legacy) for paper embedding",
                    len(_early_charts),
                )
        except Exception as _chart_exc:
            logger.warning("Stage 15: Early chart generation failed: %s", _chart_exc)

    return StageResult(
        stage=Stage.RESULT_ANALYSIS,
        status=StageStatus.DONE,
        artifacts=tuple(artifacts),
        evidence_refs=tuple(f"stage-15/{a}" for a in artifacts),
    )
...
# ---------------------------------------------------------------------------
# Agent-mode requirements gate (used by stage 16 for collider_agent /
...
def _read_requirements_from_manifest(run_dir: Path) -> list[dict[str, object]]:
    ...
    Lookup order:
      1. ``run_dir/stage-10/requirements.json``  (written by prepare_run.py
    ...
    """
    candidates = (
        run_dir / "stage-10" / "requirements.json",
    ...
    for path in candidates:
        ...
        try:
            ...
            continue
        # stage-10/requirements.json stores the list directly; manifest snapshots
...
def _read_experiment_summary(run_dir: Path) -> dict[str, object]:
    ...
    if not p.is_file():
        for sd in sorted(run_dir.glob("stage-15*/experiment_summary.json"), reverse=True):
            p = sd
            break
    if p and p.is_file():
        try:
...
def _read_agent_results_canonical(run_dir: Path) -> dict[str, object]:
    """Read the agent-written results.json from the stage-13 sandbox workspace.
    ...
    workspace_roots = (
        run_dir / "stage-13" / "runs" / "workspace" / "sandbox",
        run_dir / "stage-13" / "runs",
        run_dir / "stage-14" / "experiment_final",
    )
...
def _write_repair_prompt(run_dir: Path, delta_feedback: str, verdict: dict[str, object]) -> Path:
    """Write a REPAIR_PROMPT.md so the next stage-13 sandbox run consumes it.

    Critical placement note: the runner's ``_version_rollback_stages`` renames
    ``run_dir/stage-13/`` to ``stage-13_v{N}/`` before the agent re-runs, which
    means anything written under stage-13/ is archived (not seen by the fresh
    ...
    ``run_dir/stage-13/runs/workspace/sandbox`` layout) — see
    ...
    body = (
        ...
        "workspace artifacts (under stage-13_v{N}/runs/workspace/sandbox/) are "
    ...
    # Also drop a copy into the live stage-13 workspace as a backup for runs
    ...
    sandbox_ws = run_dir / "stage-13" / "runs" / "workspace" / "sandbox"
...
def _format_agent_decision_md(
    ...
    if rerun_triggered:
        lines += [
            "Requirements unmet — REPAIR_PROMPT.md written to stage-13 sandbox "
...
def _agent_requirements_decision(
    *,
    ...
    """Run the requirements judge and produce a stage-16 decision.
    ...
    if not requirements:
        logger.info("Stage 16: agent-mode but no manifest requirements declared — falling through")
        return None
    if llm is None:
        logger.warning("Stage 16: agent-mode requirements gate requires an LLM client; falling through")
        return None
    ...
    return StageResult(
        ...
        evidence_refs=("stage-16/decision.md",),
        decision=decision,
    )
...
def _execute_research_decision(
    ...
    *,
    ...
    if _diag_path.exists():
        try:
            ...
            if not _sufficient:
                ...
                logger.info(
                    "Stage 16: Injected experiment diagnosis — mode=%s, issues=%s",
    ...
    # alphabetically-last stage-15* (which could be a stale versioned dir)
    _exp_sum_path = run_dir / "experiment_summary_best.json"
    if not _exp_sum_path.is_file():
        _exp_sum_path = None
        for _s15 in sorted(run_dir.glob("stage-15*/experiment_summary.json"), reverse=True):
            _exp_sum_path = _s15
            break
...
"""
    ...
    if decision is None:
        ...
        logger.warning(
            "Stage 16: model decision response contained no recognized keyword — pausing pipeline"
        )
        return StageResult(
            stage=Stage.RESEARCH_DECISION,
            ...
            evidence_refs=("stage-16/decision.md", "stage-16/decision_structured.json"),
            decision="undecided",
        )
