...
class ColliderAgentSandbox:
    ...
    def run_project(
        ...
        *,
        ...
        existing or delta plan markdown.  This is what stage-15 repair loops
    ...
    def _prepare_workspace(self, prompt_text: str) -> Path:
        ...
        #   2. run_dir/REPAIR_PROMPT.md    — preserved across stage-13 archival
        ...
        if followup_delta:
            prefix = (
                ...
                "requirements.  This is the FINAL retry — the next stage-16 gate\n"
    ...
    def _merge_incremental_results(self, workspace: Path) -> None:
        ...
        Looks for the most recent ``stage-13_v{N}/runs/results.json`` two levels
        up from the workspace
        (workspace = ``run_dir/stage-13/runs/collider_workspace``).
        ...
        snapshots = sorted(
            (p for p in run_dir.glob("stage-13_v*") if p.is_dir()),
            key=lambda p: int(p.name.replace("stage-13_v", "") or "0"),
        )
        if not snapshots:
            logger.debug("Incremental merge: no stage-13_v* snapshot found")
            return
    ...
    def _build_metrics(
        ...
        here so stage 15 / the bench rubric see them.
        """
