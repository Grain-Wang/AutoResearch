"""ResearchClaw CLI — 16-stage experiment pipeline + per-step toolset."""
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    # with it — otherwise Stage 11/18/22 may keyword-detect a different
...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_p = sub.add_parser(
        "run", help="Run the 16-stage experiment pipeline (or batch a range)"
    )
    ...
    _ = run_p.add_argument(
        "--from-stage", help="Start from a specific stage (e.g. RESULT_ANALYSIS)"
    )
    _ = run_p.add_argument(
    ...
    _ = run_p.add_argument(
        ...
        action="store_true",
        help=(
            "Enable non-destructive Stage 13 re-entry for hep-ph collider runs: "
    ...
    _ = run_p.add_argument(
        ...
        help=(
            ...
            "supplies config-level defaults (experiment.mode, docker_image, "
    ...
    prof_p = sub.add_parser(
        ...
        description=(
            ...
            "Docker image, pip packages). `create` launches an interactive "
            "wizard with autocomplete suggestions."
        ),
    )
    ...
    # Experiment toolset: run each stage as an independent command
    tools_p = sub.add_parser(
        "tools",
        help="Experiment toolset: run pipeline stages individually",
        description=(
            "Treat the 16-stage experiment pipeline as a toolbox. Each step is "
            "an independent command that reads prior stage artifacts from "
            "--run-dir, executes one stage, and writes its outputs. Use "
            "'tools list' / 'tools status' to inspect the workspace."
        ),
    )
    _ = tools_p.add_argument(
        "tools_action",
        choices=TOOLS_STEPS + ["list", "status", "init"],
        help="Step to run: list | status | init | <step> "
             "(topic, decompose, search, collect, screen, extract, synthesize, "
             "reproduce, hypothesize, design, codegen, plan, experiment, refine, "
             "analyze, decide)",
    )
    _ = tools_p.add_argument(
        "--run-dir", default=None,
        help=f"Shared workspace directory (default: {Path('artifacts') / 'workspace'})",
    )
    _ = tools_p.add_argument(
        "--config", "-c", default=None,
        help="Config file (default: auto-detect config.arc.yaml / config.yaml)",
    )
    _ = tools_p.add_argument(
        "--topic", "-t", default=None, help="Research topic (overrides config)"
    )
    _ = tools_p.add_argument(
        "--domains", nargs="+", default=None,
        help="Research domains (overrides config)",
    )
    _ = tools_p.add_argument(
        "--auto-approve", action="store_true",
        help="Auto-approve gate stages (literature_screen / baseline_reproduce / experiment_design)",
    )
    _ = tools_p.add_argument(
        "--baseline", default=None,
        help="reproduce: strongest baseline name to reproduce (written as a hint)",
    )
    ...
    if action == "list":
        ...
        for entry in entries:
            ...
            print(
                ...
                f"{entry['display_name']:<40}  mode={mode}"
            )
