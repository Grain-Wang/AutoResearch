...
Integrates between Stage 15 (result_analysis) and Stage 16 (research_decision).
"""
...
def select_best_results(
    ...
    # Check main stage-15
    main_summary = _try_load_summary(run_dir / "stage-15" / "experiment_summary.json")
    if main_summary:
    ...
    for i in range(1, MAX_REPAIR_CYCLES + 1):
        path = run_dir / f"stage-15_repair_v{i}" / "experiment_summary.json"
...
def run_repair_loop(
    ...
    After Stage 15 diagnosis finds quality issues:
    ...
    # and force a wasted pivot. Skip the loop and let Stage 16 see exactly
    # what Stage 15 produced.
    ...
    for cycle in range(1, max_cycles + 1):
        ...
        repair_dir = run_dir / f"stage-15_repair_v{cycle}"
...
def _load_experiment_summary(run_dir: Path) -> dict | None:
    ...
    for candidate in sorted(run_dir.glob("stage-15*/experiment_summary.json"), reverse=True):
        try:
...
def _load_refinement_log(run_dir: Path) -> dict | None:
    ...
    for candidate in sorted(run_dir.glob("stage-14*/refinement_log.json"), reverse=True):
        try:
            return json.loads(candidate.read_text(encoding="utf-8"))
        except (json.JSONDecodeError, OSError):
            continue
...
def _load_experiment_code(run_dir: Path) -> dict[str, str]:
    ...
    Prefers: stage-14/experiment_final/ → stage-11/experiment/ → stage-11/*.py
    """
    code: dict[str, str] = {}

    # Try refined code first
    for refine_dir in sorted(run_dir.glob("stage-14*/experiment_final"), reverse=True):
    ...
    # Fall back to stage-11 experiment directory
    for exp_dir in sorted(run_dir.glob("stage-11*/experiment"), reverse=True):
        if exp_dir.is_dir():
    ...
    # Last resort: any .py files in stage-11*
    for stage_dir in sorted(run_dir.glob("stage-11*"), reverse=True):
...
def _load_experiment_plan(run_dir: Path) -> dict | None:
    ...
    for candidate in sorted(run_dir.glob("stage-10*/experiment_design.json"), reverse=True):
        try:
            return json.loads(candidate.read_text(encoding="utf-8"))
        except (json.JSONDecodeError, OSError):
            continue
...
def _collect_experiment_output(run_dir: Path) -> tuple[str, str]:
    ...
    for stage_dir in sorted(run_dir.glob("stage-15*")):
...
def _build_experiment_summary_from_run(
    ...
    for key, value in metrics.items():
        ...
        if len(parts) >= 3:
            ...
            # BUG-199: Stage 14 refinement produces 2-part keys
