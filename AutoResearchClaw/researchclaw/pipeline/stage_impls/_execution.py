"""Stages 12-14: Resource planning, experiment execution, and iterative refinement."""
    ...
    *,
    ...
    if llm is not None:
        ...
        if (
            ...
            logger.warning(
                "Stage 12: model response missing/empty 'tasks' list "
    ...
    return StageResult(
        ...
        evidence_refs=("stage-12/schedule.json",),
    )


def _estimate_stage13_footprint_bytes(run_dir: Path) -> int:
    """Sum the on-disk size of stage-13 and any stage-13_v* siblings."""
    total = 0
    for d in run_dir.glob("stage-13*"):
        if not d.is_dir():
            continue
        for p in d.rglob("*"):
            try:
...
def _execute_experiment_run(
    ...
    *,
    ...
    if mode == "collider_agent":
        ...
        # Incremental re-entry: snapshot prior workspace under stage-13_v{N}
        ...
        if (
            ...
            existing_versions = sorted(
                p for p in run_dir.glob("stage-13_v*")
                if p.is_dir() and p.name.replace("stage-13_v", "").isdigit()
            )
            next_v = (
                int(existing_versions[-1].name.replace("stage-13_v", "")) + 1
                if existing_versions
                else 1
            )
            snap_dir = run_dir / f"stage-13_v{next_v}"
            try:
                ...
                _footprint = _estimate_stage13_footprint_bytes(run_dir)
                _GB = 1024 * 1024 * 1024
                if _footprint > 20 * _GB:
                    logger.warning(
                        "Incremental footprint cumulative across stage-13*/ is "
                        "%.1f GB. Consider `rm -rf %s/stage-13_v*` to reclaim space.",
                        _footprint / _GB,
                        run_dir,
                    )
    ...
    if mode in ("sandbox", "docker"):
        ...
        if run_status == "completed" and result.elapsed_sec and result.elapsed_sec < 5.0:
            logger.warning(
                "Stage 13: Experiment completed in %.2fs — benchmark may be trivially easy. "
        ...
        if structured_results is None and effective_metrics:
            ...
            logger.info("Stage 13: Auto-generated results.json from stdout metrics (%d keys)", len(effective_metrics))
        (runs_dir / "run-1.json").write_text(
        ...
        if result.timed_out or (result.elapsed_sec and result.elapsed_sec > config.experiment.time_budget_sec * 0.9):
            ...
            logger.warning(
                "Stage 13: %s", _time_budget_warning["warning"]
            )
                ...
                for _cname, _cdata in _sr_conditions.items():
                    if isinstance(_cdata, dict):
                        ...
                        if isinstance(_seeds_run, (int, float)) and 0 < _seeds_run < 3:
                            logger.warning(
                                "Stage 13: Condition '%s' ran only %d seed(s) — "
    ...
    if mode in ("sandbox", "docker"):
        ...
        if not _has_real_metrics and run_status == "failed":
            logger.error(
                "Stage 13: Experiment FAILED and produced zero real metrics. "
        ...
        if not _has_real_metrics and _stdout_has_failure:
            logger.error(
                "Stage 13: Experiment crashed (failure signals in stdout) with zero "
        ...
        if (
            ...
            logger.error(
                "Stage 13: Experiment 'completed' in %.1fs with zero real metrics "
...
def _execute_iterative_refine(
    ...
    *,
    ...
    # Stage 13 and wrote a canonical results.json.  "Refining" python
    ...
    if config.experiment.mode in ("collider_agent", "biology_agent", "stat_agent"):
        ...
        logger.info(
            "Stage 14: Skipping iterative refinement in %s mode "
            "(%s pipeline completed in Stage 13)",
        ...
        # Copy Stage 13 run artifacts into experiment_final/ for downstream stages
        ...
        if runs_artifact and Path(runs_artifact).is_dir():
            ...
            (final_dir / f"{agent_label}_results.md").write_text(
                f"# {agent_pretty} Results\n\nExperiment executed via {agent_pretty} in Stage 13.\n",
                encoding="utf-8",
            )
        ...
        log: dict[str, Any] = {
            ...
            "skip_reason": (
                ...
                f"{agent_pretty} ran the full pipeline in Stage 13"
            ),
    ...
    # (BUG-06 fix: no longer auto-override, since Stage 10 and 12 now
    ...
    # BUG-58: When PIVOT rolls back to Stage 14, prefer the best refined code
    # from a previous cycle (stage-14_vX/experiment_final/) over the original
    # unrefined code (stage-13/experiment/ or stage-11/experiment/).
    ...
    _prev_refine_dirs = sorted(
        run_dir.glob("stage-14_v*/experiment_final"),
    ...
    if llm is None:
        logger.info("Stage 14: LLM unavailable, saving original experiment as final")
        final_dir = stage_dir / "experiment_final"
    ...
    if _exp_plan_text and run_summaries:
        ...
        if not _has_condition_labels and _exp_plan_text.strip():
            ...
            logger.info(
                "Stage 14: condition coverage gap detected, injecting multi-condition hint"
            )
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if len(_valid_metrics) >= 2:
            ...
            if _saturated:
                ...
                logger.warning("Stage 14: metric saturation detected, injecting difficulty upgrade hint")
        ...
        if prior_timed_out and baseline_metric is None:
            ...
            logger.warning(
                "Stage 14: injecting timeout-aware prompt (attempt %d)",
        ...
        try:
            ...
            if "ACP prompt timed out after" in str(exc):
                logger.warning(
                    "Stage 14: ACP prompt timed out during iteration %d; pausing for resume",
                    iteration,
                )
        ...
        if (
            ...
            if _ep_idx != -1:
                ...
                logger.info(
                    "Stage 14 iter %d: restored entry point stripped by LLM "
        ...
        if not validation.ok:
            ...
            logger.info(
                "Stage 14 iteration %d validation failed: %s",
                iteration,
                validation.summary(),
            )
            irp = _pm.sub_prompt(
            ...
            try:
                ...
                if "ACP prompt timed out after" in str(exc):
                    logger.warning(
                        "Stage 14: ACP repair prompt timed out during iteration %d; pausing for resume",
                        iteration,
                    )
                    return _pause_refinement(
                        reason=str(exc),
        ...
        if validation.ok and config.experiment.mode in ("sandbox", "docker"):
            ...
            # R19-1: Store stdout (capped) so PAIRED lines survive for Stage 15
            ...
            if rerun.timed_out:
                ...
                timeout_refine_attempts += 1
                logger.warning(
                    "Stage 14 iteration %d: sandbox timed out after %.1fs",
                    iteration,
                    rerun.elapsed_sec,
                )
                ...
                if not rerun.metrics and rerun.stdout:
                    ...
                    if partial:
                        ...
                        iter_record["metric"] = metric_val
                        logger.info(
                            "Stage 14 iteration %d: recovered %d partial metrics from timeout stdout",
                            iteration,
                            len(partial),
                        )
            ...
            if runtime_issues:
                ...
                logger.info(
                    "Stage 14 iteration %d: runtime issues detected: %s",
                    iteration,
                    runtime_issues[:200],
                )
                ...
                try:
                    ...
                    if "ACP prompt timed out after" in str(exc):
                        logger.warning(
                            "Stage 14: ACP runtime-repair prompt timed out during iteration %d; pausing for resume",
                            iteration,
                        )
                        return _pause_refinement(
                            reason=str(exc),
        ...
        if consecutive_no_metrics >= 3:
            ...
            logger.warning("Stage 14: Aborting after %d consecutive iterations without metrics", consecutive_no_metrics)
            break
        ...
        if no_improve_streak >= 2:
            ...
            logger.info(
                "Stage 14 converged after %d iterations (no improvement streak=%d)",
                iteration,
                no_improve_streak,
            )
            break
