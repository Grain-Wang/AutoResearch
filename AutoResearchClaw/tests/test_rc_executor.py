...
def test_execute_stage_creates_stage_dir_writes_artifacts_and_meta(
    ...
    agents_file = run_dir.parent / "AGENTS.md"
    agents_file.write_text("EXECUTOR_WORKSPACE_RULE", encoding="utf-8")
    ...
    assert (run_dir / "stage-01" / "AGENTS.md").read_text(
        encoding="utf-8"
    ) == "EXECUTOR_WORKSPACE_RULE"
    agents_context = json.loads(
        (run_dir / "stage-01" / "agents_context.json").read_text(encoding="utf-8")
    )
    assert agents_context["source"] == str(agents_file)
    assert len(agents_context["sha256"]) == 64
    assert len(fake_llm.calls) == 1
    assert "EXECUTOR_WORKSPACE_RULE" in fake_llm.calls[0][0]["content"]
