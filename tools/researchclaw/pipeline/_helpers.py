...
def _read_best_analysis(run_dir: Path) -> str:
    """BUG-225: Read analysis.md from the best Stage 15 iteration.

    Prefers ``analysis_best.md`` at run root (written by
    ``_promote_best_stage15``) over ``_read_prior_artifact("analysis.md")``
    which may pick a degenerate non-versioned stage-15 directory.
    """
...
def _read_prior_artifact(run_dir: Path, filename: str) -> str | None:
    # R14-2: Sort so non-versioned dirs (stage-14) come before versioned (stage-14_v1).
