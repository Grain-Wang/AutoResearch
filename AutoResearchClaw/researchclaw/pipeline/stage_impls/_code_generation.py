...
def _execute_collider_plan_generation(
    ...
    *,
    ...
    """Stage 11 (collider_agent mode): generate a ColliderAgent physics prompt.

    Reads the experiment design plan from Stage 10 and uses the LLM to
    ...
    directory.  Stage 13 reads this file and invokes Claude Code with the
    ...
    if llm is not None:
        try:
            ...
            logger.warning("Stage 11 (collider_agent): LLM call failed (%s) — using fallback plan", exc)
    ...
    logger.info("Stage 11 (collider_agent): wrote physics prompt to %s", plan_path)
    ...
    # Satisfy Stage 11 contract (output_files requires "experiment/" and
    ...
    spec_md = (
        ...
        f"Stage 13 will invoke the Claude Code CLI with the ColliderAgent skills\n"
    ...
    return StageResult(
        ...
        evidence_refs=("stage-11/collider_plan.md",),
    )
...
def _execute_code_generation(
    ...
    *,
    ...
    # --- BA: Inject BenchmarkAgent plan from Stage 10 ---
    _bp_path = None
    for _s10_dir in sorted(run_dir.glob("stage-10*"), reverse=True):
        _candidate = _s10_dir / "benchmark_plan.json"
    ...
    for fname, code in list(files.items()):
        ...
        if _rl_errors:
            for _rl_err in _rl_errors:
                logger.error("Stage 11: %s (in %s)", _rl_err, fname)
    ...
    if not all_valid:
        ...
        if _has_critical:
            logger.error(
                "Stage 11: CRITICAL validation issues remain after %d repair "
    ...
    for fname, code in list(files.items()):
        if fname.endswith(".py"):
            ...
            if n_fixes > 0:
                ...
                logger.info(
                    "Stage 11: auto-fixed %d UnboundLocalError risk(s) in %s",
                    n_fixes, fname,
                )
    if _total_ub_fixes:
        logger.info(
            "Stage 11: auto-fixed %d total UnboundLocalError risks", _total_ub_fixes
        )
    ...
    if critical_deep and llm is not None:
        logger.info(
            "Stage 11: %d critical code issues found — triggering repair cycle",
            len(critical_deep),
        )
        ...
        try:
            ...
            if repaired and "main.py" in repaired:
                ...
                logger.info(
                    "Stage 11: Deep repair fixed %d/%d critical issues",
    ...
    # --- P1.4: LLM Code Review (Stage 11.5) ---
        ...
        try:
            ...
            if isinstance(review_data, dict):
                ...
                if critical_issues and review_score <= 4:
                    logger.warning(
                        "Stage 11 code review: score=%d, %d critical issues — "
                        "attempting fix",
                    ...
                    try:
                        ...
                        if fixed_files and "main.py" in fixed_files:
                            ...
                            for fname, code in files.items():
                                (exp_dir / fname).write_text(code, encoding="utf-8")
                            logger.info(
                                "Stage 11: Code fixed after review "
    ...
    if llm is not None:
        ...
        try:
            ...
            if isinstance(align_data, dict) and not align_data.get("aligned", True):
                ...
                logger.warning(
                    "Stage 11: Topic-experiment MISALIGNMENT detected: %s",
                    alignment_note,
                )
                ...
                for _regen_attempt in range(1, _max_regen + 1):
                    logger.info(
                        "Stage 11: Alignment regen attempt %d/%d",
                    ...
                    if not regen_files or "main.py" not in regen_files:
                        logger.warning(
                            "Stage 11: Regen attempt %d produced no main.py",
                            _regen_attempt,
                        )
                        continue
                    files = regen_files
                    ...
                    if isinstance(recheck_data, dict) and recheck_data.get("aligned", False):
                        ...
                        logger.info(
                            "Stage 11: Code aligned after regen attempt %d",
                            _regen_attempt,
                        )
                        break
                    else:
                        ...
                        logger.warning(
                            "Stage 11: Regen attempt %d still misaligned: %s",
    ...
    if llm is not None and main_code and "condition" in main_code.lower():
        try:
            ...
            if isinstance(abl_data, dict) and abl_data.get("has_duplicates"):
                logger.warning(
                    "Stage 11: Duplicate ablation conditions detected: %s",
                ...
                try:
                    ...
                    if repaired_files and "main.py" in repaired_files:
                        ...
                        for fname, code in files.items():
                            (exp_dir / fname).write_text(code, encoding="utf-8")
                        logger.info(
                            "Stage 11: Ablation repair applied — "
...
"""
    ...
    if not alignment_ok:
        logger.error(
            "Stage 11: Persistent topic-experiment misalignment after all "
