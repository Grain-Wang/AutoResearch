...
`RECOVER_TWO_REAL_DATASETS`。该选择保持强 CCF-C / 弱 CCF-B 的算法论文目标，但不恢复已经失败的 `NYUv2 + current frozen KITTI source` 分支。当前 Claim-M 唯一合法状态为 `STOPPED_CURRENT_DATA_BRANCH`，直到新的 Step003 authorization 明确返回 PASS。
...
## Allowed work while the formal branch is stopped
...
- 用 released checkpoint 运行 004-A H-sensitivity diagnostic；
...
diagnostic corpus 与 H-sensitivity 只能回答“caption intervention 是否有效、问题是否真实”，不能回答 D0-relative fallback、Claim-F 或 Claim-M。其 image/caption/template hashes 不得被后续 dev/internal-test 使用。

## Forbidden work until authorization PASS
...
- 恢复 exclusive GPU queue，或启动上方明确授权的 004-A 之外的科学 GPU 任务。
...
3. NYUv2 diagnostic 若 machine-check/independent precision `<0.95`，或所有局部 error families 的 H-sensitivity cluster CI 均不支持退化，则停止 CoVoL 问题主张，即使第二数据集可用也不进入 Main-PR。
