...
_GATE_STAGES = frozenset({5, 8, 10})

_PHASE_BOUNDARIES = frozenset({
    2,   # End of Phase A (Research Scoping)
    6,   # End of Phase B (Literature Discovery)
    8,   # End of Phase C (Knowledge Synthesis & Defect Baseline)
    12,  # End of Phase D (Hypothesis & Experiment Design)
    14,  # End of Phase E (Experiment Execution)
    16,  # End of Phase F (Analysis & Decision)
})

# Co-pilot mode: critical stages needing deep collaboration
_COPILOT_COLLABORATION_STAGES = frozenset({7, 9, 10})
_COPILOT_APPROVAL_STAGES = frozenset({5, 8, 10, 16})
_COPILOT_PAUSE_AFTER_STAGES = frozenset({1, 2, 3, 11, 14})
_COPILOT_STREAM_STAGES = frozenset({13})
