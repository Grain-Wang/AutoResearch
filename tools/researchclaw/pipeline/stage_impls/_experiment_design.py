...
def _execute_experiment_design(
    ...
    *,
    ...
    try:
        ...
        # Persist domain profile for Stage 11
    ...
    # BUG-12: Fallback 4 — extract method/baseline names from Stage 9 hypotheses
    if plan is None:
    ...
    if not any(_normalized.values()):
        ...
        logger.warning(
            "Stage 10: model plan parsed but missing required content keys — pausing pipeline"
        )
        return StageResult(
            ...
            evidence_refs=("stage-10/plan_meta.json",),
    ...
    if _total > _max_conditions:
        logger.warning(
            "Stage 10: Plan has %d conditions (limit %d for %ds budget). "
            "Trimming to fit.",
        ...
        if len(_proposed) > _proposed_count:
            ...
            logger.info(
                "Stage 10: Trimmed proposed methods %d → %d",
    ...
    return StageResult(
        ...
        evidence_refs=("stage-10/exp_plan.yaml",),
    )
